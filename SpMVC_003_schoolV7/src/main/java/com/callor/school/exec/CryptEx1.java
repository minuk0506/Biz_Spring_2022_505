 ...
 * 	이후 로그인을 시도하면 table 에 저장된 비번을 다시 복호화하여 로그인 정보와 비교하는 것
 * 
 ...
 * 	회원가입할 때 입력된 비번을 암호화하여 table 에 저장하고
 * 
 * 이후 로그인을 시도하면
 * 입력된 비번을 다시 암호화하여 table 에 저장된 값과 비교한다.
 * 복호화 기능이 존재하지 않으므로
 * 암호화 할때 사용된 기법이 노출되지 않고, 다소 안전하게 작동할 수 있다.
 * 
