...
public class UserContollerV2 {
	...
	public UserContollerV2(@Qualifier("userServiceV2") UserService userService) {
	 ...
	 * 
	 * 임의로 이 정보를 삭제하려면
	 * session.removeAttribute() 메서드를 사용하여 USER라는 변수를 삭제 해 주어야 한다
	 */
	 ...
	 * /user/idcheck?username=minuk
	 * 
	 * fetch(`${rootPath}/user/idcheck/${username.value}`)
	 * 만약 username 에 minuk 입력했으면
	 * /user/idcheck/minuk 처럼 요청 URL 만들어서 요청을 수행하라
	 * 
	 * 
	 * id 를 email 주소로 사용할때 PathVarriable 로 받을 경우
	 * dot(.) 이후의 문자열을 잘라버리는 현상이 있다
	 * 이때는 정규식(Rexp) 을 사용하여 dot(.) 이후 문자열을 포함하여
	 * 변수에 저장하도록 변수를 수정
	 * 		{username:.+} 형식으로 지정한다
	 */
	@ResponseBody
