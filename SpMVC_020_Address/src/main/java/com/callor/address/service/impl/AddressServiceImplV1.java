...
//	}
	...
	// 프로젝트가 시작될때 table 을 자동으로 생성하기 위한 자동 실행 method
	@Autowired
	...
	/*
	 * 이 method 에서 dao의 create_addr_table() method 를 호출하여 table 을 생성해도 된다
	 * 
	...
	public void searchAndPage(Model model, SearchPage searchPage) {
		...
		// 보여질 데이터 리스트 개수
		searchPage.setListPerPage(6);
		
		// 하단의 페이지 번호 개수
		searchPage.setPageNoConut(6);
		...
		long startPageNo = ((searchPage.getCurrentPageNo() -1)
				/ searchPage.getPageNoConut()) * searchPage.getPageNoConut() + 1;
		
		long endPageNo = startPageNo + searchPage.getPageNoConut() - 1;
		
		searchPage.setStartPageNo(startPageNo);
		searchPage.setEndPageNo(endPageNo);
		searchPage.setLimit(searchPage.getPageNoConut());
		searchPage.setOffset(searchPage.getCurrentPageNo() * searchPage.getPageNoConut());
