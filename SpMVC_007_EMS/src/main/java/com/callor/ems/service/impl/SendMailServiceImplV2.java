...
public class SendMailServiceImplV2 extends SendMailServiceImplV1 {
	...
	/*
	 * spring 프로젝트에서 src/main/resources 폴더에 저장된 파일들에
	 * 접근하기 위한 보조 도구
	 */
	...
	public void sendMail(EmsVO emsVO, UserVO userVO) {
		...
		// 네트워크를 통해 전송하기 때문에 try catch
		MimeMessageHelper mHelper = null;
		try {
			/*
			 * 두번째 전달 값 false : 순수 text 만 전송가능 text/plan type 으로 전송 true : html 코드를 포함,
			 * 첨부파일도 포함 가능, text/html, multipart/form-data
			 */
			mHelper = new MimeMessageHelper(message, true, "UTF-8");
			mHelper.setFrom("minuk0506@naver.com", emsVO.getE_from_name());
			String[] sendTo = { emsVO.getE_to_email(), "minuk0506@naver.com"};
